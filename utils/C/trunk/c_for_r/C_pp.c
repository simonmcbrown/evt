...
/* Originally written by Simon Brown in 2003 during a very hot summer in Exeter  */
...
/***********************************************************************/
void
c_inquire_pp_file(fname,count,nrow,npt,quiet)
char**		fname;
int*		count;
int*		nrow;
int*		npt;
...
*/
	...
	*npt   = ntohl(pp_ptr->head.LBNPT);
	...
	if (*quiet == 0) Rprintf("%d fields read, %d rows %d points\n",*count,*nrow,*npt);
	...
	while (!(feof(fin)) & (n2<(*nfields)))
	  ...
	  if (n == (idxfields[n2]-1) )
		...
		byte_swap_pp(pp_ptr,READPP);
					...
					for(i=0; i<=pp_ptr->head.LBLREC -1 -pp_ptr->head.LBEXT ; i++)
						 data[n2*(pp_ptr->head.LBLREC-pp_ptr->head.LBEXT) +i] = pp_ptr->data[i];
					Free(pp_ptr->data);
...
*/
	...
	int			i,j, n, nfields, temp1, localext;
...
/*pp_ptr->head.LBEXT = ntohl(*(LBEXT +n));  SJB PP_IO code does not support extra data */
if (*(LBEXT +n) == 0) {
    pp_ptr->head.LBEXT  = ntohl(*(LBEXT +n));
    pp_ptr->head.LBLREC = ntohl(*(LBLREC +n));
} else {
    pp_ptr->head.LBLREC = ntohl(*(LBLREC +n) - *(LBEXT +n));
    pp_ptr->head.LBEXT  = ntohl(0);
}
pp_ptr->head.LBEXT = ntohl(0);
...
/* LBLREC has to be adjusted to ignore LBEXT */
	swap_4byte(     (int *)(data+( (LBLREC[n]-LBEXT[n])*n )),    (LBLREC[n]-LBEXT[n])           );

  /*fwrite((data+  LBLREC[n]*n),              sizeof(float),  LBLREC[n], fout);*/
  fwrite((data+ (LBLREC[n]-LBEXT[n])*n),    sizeof(float),  LBLREC[n], fout);
	...
	while (!(feof(fin)) & (n2<(*nfields)))
	  ...
	  if (n == (idxfields[n2]-1) )
		...
		byte_swap_pp(pp_ptr,READPP);
					...
					data[n2] = *(pp_ptr->data + ((*irow)-1)*(pp_ptr->head.LBNPT) + (*ipt) -1);
...
/*      reads all pp fields:  header, data and buffer bytes
		...
		21/06/2011 fixed error where byte swap went beyond allocated data
*/
	  ...
	  if (n == (idxfields[n2]-1) )
		...
		byte_swap_pp(pp_ptr,READPP);
					...
					for(i=0; i<=pp_ptr->head.LBNPT -1; i++)
						data[n2*pp_ptr->head.LBNPT +i] = *(pp_ptr->data + ((*irow)-1)*(pp_ptr->head.LBNPT) +i);
...
/***********************************************************************/
void
c_read_row_mk1_pp_file(int *irow,
					char **fname,
					int *count,
					int *idxfields,
					int *LBYR,
...
*/
	...
	/* I dont think we need to allocate memory for the data in pp_ptr->data as it is just pointing to an address in data_buff 		pp_ptr->data = (float *) Calloc(ntohl(pp_ptr->head.LBNPT)*sizeof(float),float);
	*/
...
/***********************************************************************/
void
c_read_point_mk1_pp_file(int *ipt,
					int *irow,
					char **fname,
					int *count,
					int *idxfields,
