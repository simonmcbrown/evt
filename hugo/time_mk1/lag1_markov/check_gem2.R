...
source("/home/h03/hadsx/extremes/R/libs/simon_util.R")

# general understanding of tmaxs

seas_length <- 90

st.ant   <- "~/extremes/tawn/hugo/heatwave/gem2/orleans/nat_1234_allyr_daily_tmax_1950_2011.csv"
st.all   <- "~/extremes/tawn/hugo/heatwave/gem2/orleans/ghg_1234_allyr_daily_tmax_1950_2005.csv"
psout    <- '~/extremes/tawn/hugo/time_mk1/check_gem2_orleans.ps'
tit0     <- "GEM2 Daily Max Temperature: Orleans"

#st.ant   <- "~/extremes/tawn/hugo/heatwave/gem2/toulouse/nat_1234_allyr_daily_tmax_1950_2011.csv"
#st.all   <- "~/extremes/tawn/hugo/heatwave/gem2/toulouse/ghg_1234_allyr_daily_tmax_1950_2005.csv"
#psout    <- '~/extremes/tawn/hugo/time_mk1/check_gem2_toulouse.ps'
#tit0     <- "GEM2 Daily Max Temperature: Toulouse"
...
for(d in 1:36) cll[d] <- mean(txl[sort(findall(idec[d,],doyl))])

clt <- NULL
#for(d in 1:36) clt[d] <- mean(txt[doyt == idec[d,]])
for(d in 1:36) clt[d] <- mean(txt[sort(findall(idec[d,],doyt))])
...
postscript(psout,horiz=T)
    ...
    legend(0,25,c('GEM1 GHG','GEM1 NAT'),col=c(1,2),pch=1)
    ...
    x0 <- 180:230
    plot(x0,txl[x0],ylim=c(10,50),cex=.3, main='Summertime daily Tmax + medians',ylab='Degrees C',xlab='Day of year')
    ...
    for(y in unique(yl)) {
        a <- which(yl==y)
        b <- findall(x0,doyl[a])
        points(rep(x0,4),txl[sort(a[b])],cex=.3)
        a <- which(yt==y)
        b <- findall(x0,doyt[a])
        points(rep(x0,4)+.4,txt[sort(a[b])],cex=.3,col=3)
     }
    ...
    stxl <- sort(txl[ijja])
    stxt <- sort(txt[ijja])
    plot(stxl-stxt,typ='l',ylab='GHG - NAT', main='QQ Plot summertime daily Tmax differences')
    n <- length(stxt)
