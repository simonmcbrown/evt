# source("hist_sim_hw_sev.R")

# plot histograms of severity for each duration

#tit0    <- "CM3 Heatwave severity: Central France"
#psout   <- 'hist_severity_cm3.ps'
...
#tit0    <- "GEM1 Heatwave severity: Orleans"
#psout   <- 'hist_severity_orleans_gem1.ps'
#pathin  <- '/project/extremes1/hadsx/RSave.data/'
#strinH0 <- 'orleans_gem1_ant_temp_dat_sim_1e5_'
#strinC0 <- 'orleans_gem1_all_temp_dat_sim_1e5_'
#leg0    <- c('ANT','ALL')

#tit0    <- "GEM1 Heatwave severity: Toulouse"
#psout   <- 'hist_severity_toulouse_gem1.ps'
#pathin  <- '/project/extremes1/hadsx/RSave.data/'
#strinH0 <- 'toulouse_gem1_ant_temp_dat_sim_1e5_'
#strinC0 <- 'toulouse_gem1_all_temp_dat_sim_1e5_'
#leg0    <- c('ANT','ALL')

tit0    <- "GEM2 Heatwave severity: Orleans"
psout   <- 'hist_severity_orleans_gem2_ghg_nat.ps'
pathin  <- '/project/extremes1/hadsx/RSave.data/'
...
### LOAD NAT
    ...
    for (f in list.files(path=pathin,pattern=strinC0, full.names=TRUE)[1:nfiles]) {
      ...
      #20141102# sim.eventsC.1 <- c(sim.eventsC.1,sim.events[[1]])
      sim.eventsC.1 <- c(sim.eventsC.1,sim.events[[1]]+(sim.events[[3]]*sim.events[[5]]))
    }
    thC <- sim.events[[5]]
...
### LOAD ANT
    ...
    for (f in list.files(path=pathin,pattern=strinH0, full.names=TRUE)[1:nfiles]) {
      ...
      #20141102# sim.eventsH.1 <- c(sim.eventsH.1,sim.events[[1]])
      sim.eventsH.1 <- c(sim.eventsH.1,sim.events[[1]]+(sim.events[[3]]*sim.events[[5]]))
    }
    thH <- sim.events[[5]]
...
qH <- qC <- list()
for (d in 1:25) {
    qH[[d]] <- quantile(sim.eventsH.1[idH[[d]]],p)
    qC[[d]] <- quantile(sim.eventsC.1[idC[[d]]],p)
}
...
for (d in d0) {
# histogrammes
hH <- hist(sim.eventsH.1[idH[[d]]],100,plot=F)
...
hC <- hist(sim.eventsC.1[idC[[d]]],100,plot=F)
...
#ranked qq plots
lC <- length(sim.eventsC.1[idC[[d]]])
lH <- length(sim.eventsH.1[idH[[d]]])
if (lC>lH) n <- lH else n<-lC
 plot(sort(sim.eventsH.1[idH[[d]]],dec=T)[1:1000], sort(sim.eventsC.1[idC[[d]]],dec=T)[1:1000], xlab=leg0[1],ylab=leg0[2],xlim=range(sim.eventsH.1),ylim=range(sim.eventsC.1))
lines(sort(sim.eventsH.1[idH[[d]]],dec=T)[1000:n], sort(sim.eventsC.1[idC[[d]]],dec=T)[1000:n])
abline(0,1)
}
...
postscript(psout,horiz=F)
    ...
    for (d in 1:19) {
        # histogrammes
        hHC <- hist(c(sim.eventsH.1[idH[[d]]],sim.eventsC.1[idC[[d]]]),100,plot=F)
        hH  <- hist(sim.eventsH.1[idH[[d]]],breaks=hHC$breaks,plot=F)
        hC  <- hist(sim.eventsC.1[idC[[d]]],breaks=hHC$breaks,plot=F)

        x1 <- rep(hH$breaks,each=2)
        x1 <- x1[2:(length(x1)-1)]
        y1 <- rep(hH$density,each=2)
        plot(x1,y1,type='l',main=paste('Duration =',d),ylim=c(0,max(c(y1,hC$density))), xlab='Severity', ylab='Density' )
        #y1 <- rep(hH$counts,each=2)
        #plot(x1,y1,type='l',main=paste('d =',d,':',sum(hH$counts),sum(hC$counts)),ylim=c(0,max(c(y1,hC$counts))), xlab='Severity', ylab='Count' )
        ...
        if (d==1) {
            #legend(max(c(x1,hH$breaks)),max(c(y1,hH$counts)),leg0,col=c(1,2), yjust=1, xjust=1, lty=1)
