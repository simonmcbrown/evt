...
DurSevFromSim_mk2 <- function(data,marg.obj,dep.obj,dep.obj.bwd,thresh.u,crit.lev.u,max.lb,max.ub,nsim){
  ...
  # The line below was wrong and was giving P(X<x|X>u) instead of just P(X<x) which the next line requires!
  unif.random.starts <- (1-marg.obj$par[4])+(marg.obj$par[4])*EcdfWithGpd(data=random.starts,p=marg.obj$par[2:4],u=thresh)   # Need the starting positions on uniform margins for the function
