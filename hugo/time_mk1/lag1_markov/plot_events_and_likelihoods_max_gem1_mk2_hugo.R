# source("plot_events_and_likelihoods_max_gem1_mk2_hugo.R")
...
if (PRINTSAVE) {
  postscript('drought_likelihood_max_gem1_ant_vs_nat.ps',horiz=F)
  up.2()
  }
...
### LOAD ALL
    ...
    for (f in list.files(path=pathin,pattern=strinL0, full.names=TRUE)[1:nfiles]) {
      ...
      sim.eventsL.4 <- c(sim.eventsL.4,sim.events[[4]])
    }
...
#sim.eventsL.4 <- sim.eventsL.4-273.15
...
ylim <- extend.range(c(floor(sim.events[[5]]),obs.events[[4]],sim.eventsA.4,sim.eventsL.4))
#xlim <- c(0,30)
#ylim <- c(0,20)
