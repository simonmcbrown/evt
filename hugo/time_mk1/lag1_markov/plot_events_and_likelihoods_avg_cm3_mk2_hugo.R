# source("plot_events_and_likelihoods_avg_cm3_mk2_hugo.R")
...
if (PRINTSAVE) {
  postscript('drought_likelihood_avg_cm3_ant_vs_nat.ps',horiz=F)
  up.2()
  }
...
### LOAD NAT
    ...
    for (f in list.files(path=pathin,pattern=strinN0, full.names=TRUE)[1:nfiles]) {
      ...
      #sim.eventsN.1 <- c(sim.eventsN.1,sim.events[[1]])
      sim.eventsN.1 <- c(sim.eventsN.1,sim.events[[1]]+(sim.events[[3]]*sim.events[[5]]))   # : anomaly*duration
    }
    ### convert to average severity per day
    udayN    <- sort(unique(sim.eventsN.3))
    for (d in udayN) {
      ix         <- which(sim.eventsN.3 == d)
      sim.eventsN.1[ix] <- sim.eventsN.1[ix]/d
    }
...
### LOAD ANT
  ...
  ylim <- extend.range(c(sim.eventsA.1,sim.eventsN.1))
  #xlim <- c(0,30)
  #ylim <- c(0,20)
...
#points(obs.events[[3]],obs.events[[1]],cex=1,col=1,pch=21,bg=5)
